...
def run(exp, node=None, experiment=None, time_limit=None, gpu=None, input_dir=None, exec=None, events=None, threads=None, marker=None, loaded_events=None, output_dir="", t=0, filename="run"):
...
likwid-perfctr -C { create_core_string(exp['threads'] if threads == None else threads) } -g ENERGY {'-m' if (exp['marker'] if marker == None else marker) else ''} -o {output_dir}cpu{i}.csv {exp['exec'] if exec == None else exec} --detector_file=tml_detector/trackml-detector.csv --digitization_config_file=tml_detector/default-geometric-config-generic.json --input_directory={exp['input_dir'] if input_dir == None else input_dir}/ --loaded_events {int(exp['loaded_events']) if loaded_events == None else loaded_events} --processed_events {exp['events'] if events == None else events} --cold_run_events 0 --threads {exp['threads'] if threads == None else threads}
                """
            if t == 1:
...
def start_experiment(exp, t):
    ...
    if "loaded_events" in exp:
        loaded_events = exp['loaded_events']
    else:
        loaded_events = 10
    ...
    if type(exp['input_dir']) == list:
        for i, input_dir in enumerate(exp['input_dir']):
            ...
            os.mkdir(output_dir)
            run(exp, node, experiment=name, input_dir=input_dir, output_dir=output_dir, loaded_events=loaded_events, t=t, filename=f"run{i}")
        ...
        for i, n_threads in enumerate(exp['threads']):
            ...
            output_dir = f"{output}{input_name}/"
            os.mkdir(output_dir)
            run(exp, node, experiment=name, threads=n_threads, output_dir=output_dir, loaded_events=loaded_events, t=t, filename=f"run{i}")
        ...
        for i, event in enumerate(exp['events']):
            ...
            output_dir = f"{output}{input_name}/"
            os.mkdir(output_dir)
            run(exp, node, experiment=name, events=event, output_dir=output_dir, loaded_events=loaded_events, t=t, filename=f"run{i}")
        ...
        for i, gpu in enumerate(exp['gpu']):
            ...
            output_dir = f"{output}{input_name}/"
            os.mkdir(output_dir)
            run(exp, node, experiment=name, gpu=gpu, output_dir=output_dir, t=t, loaded_events=loaded_events, filename=f"run{i}")
        ...
        for i, node in enumerate(exp['nodes']):
            ...
            output_dir = f"{output}{input_name}/"
            os.mkdir(output_dir)
            run(exp, node, experiment=name, output_dir=output_dir, t=t, loaded_events=loaded_events, filename=f"run{i}")
    elif("loaded_events" in exp and type(exp['loaded_events']) == list):
        for i, n in enumerate(exp['loaded_events']):
            input_name = f"loaded_events_{n}"
            name = exp['experiment_name'] + "-" + input_name
            output_dir = f"{output}{input_name}/"
            os.mkdir(output_dir)
            run(exp, node, experiment=name, output_dir=output_dir, t=t, loaded_events=n, filename=f"run{i}")
    else:
        run(exp, node, output_dir=output, loaded_events=loaded_events, t=t)
