...
def run(exp, node=None, experiment=None, time_limit=None, gpu=None, input_dir=None, exec=None, events=None, threads=None, marker=None, output_dir="", filename="run"):
...
def start_experiment(exp):
    ...
    if type(exp['input_dir']) == list:
        for i, input_dir in enumerate(exp['input_dir']):
            ...
            os.mkdir(output_dir)
            run(exp, node, experiment=name, input_dir=input_dir, output_dir=output_dir, filename=f"run{i}")
        ...
        for i, n_threads in enumerate(exp['threads']):
            ...
            output_dir = f"{output}{input_name}/"
            os.mkdir(output_dir)
            run(exp, node, experiment=name, threads=n_threads, output_dir=output_dir, filename=f"run{i}")
        ...
        for i, event in enumerate(exp['events']):
            ...
            output_dir = f"{output}{input_name}/"
            os.mkdir(output_dir)
            run(exp, node, experiment=name, events=event, output_dir=output_dir, filename=f"run{i}")
        ...
        for i, gpu in enumerate(exp['gpu']):
            ...
            output_dir = f"{output}{input_name}/"
            os.mkdir(output_dir)
            run(exp, node, experiment=name, gpu=gpu, output_dir=output_dir, filename=f"run{i}")
        ...
        for i, node in enumerate(exp['nodes']):
            ...
            output_dir = f"{output}{input_name}/"
            os.mkdir(output_dir)
            run(exp, node, experiment=name, output_dir=output_dir, filename=f"run{i}")
    else:
